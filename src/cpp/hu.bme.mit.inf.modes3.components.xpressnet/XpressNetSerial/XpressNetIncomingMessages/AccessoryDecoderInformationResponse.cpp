...
AccessoryDecoderInformationResponse::AccessoryDecoderInformationResponse()
    : IncomingXPNMessage()
{
...
bool AccessoryDecoderInformationResponse::recognizedMessage(std::vector<uint8_t> messageBytes)
{
    unsigned int baseAddress =
        messageBytes[1] * 4 + ((messageBytes[2] & nibbleMask) >> 4) * 2 + 1; //+1 for MultiMaus friendly address
    ...
    for(int i = 0; i < 2; i++) {
        ...
        if((status[i] > 0) && (status[i] < 3)) {
            ...
            if(changed == true) {
                std::cout << std::dec << std::setfill('0') << std::setw(2) << baseAddress + i << " " << direction
                          << std::endl;
                ProtobufTranslator::turnoutStateChanged(baseAddress + i, currentTurnoutState);
            }
        }
    }
