...
LocomotiveInformation::LocomotiveInformation()
    : IncomingXPNMessage()
{
...
bool LocomotiveInformation::recognizedMessage(std::vector<uint8_t> messageBytes)
{
    ...
    if(trainAddress != -1) {
        ...
        if(changed == true) {
            ...
            std::cout << "Is locomotive free: " << free << " Current speed step mode: " << std::dec << currentSpeedSteps
                      << std::endl;
            ...
            ProtobufTranslator::trainSpeedChanged(trainAddress, trainDirection, speed);
        }
        /*
