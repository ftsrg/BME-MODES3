...
l<j.length;
l++){var k=j[l];
if(k.handleOnlineOffline){k.close();
clearTimeout(k.response.request.id);
if(k.heartbeatTimer){clearTimeout(k.heartbeatTimer)
}}}}});
...
j<h.length;
j++){if(h[j].handleOnlineOffline){h[j].init();
h[j].execute()
}}}d=false
});
return a
