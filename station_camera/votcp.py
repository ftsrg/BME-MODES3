...
import socket
from tcpStream.TCPStream import tcpStream
from tcpStream.TCPVideoStream import tcpVideoStream

# name of the opencv window
cv_window_name = "SSD Mobilenet"

cap = cStream(src = 0, usePiCamera = False, resolution = (640, 480), framerate = 10).start()
tcpDataStream = tcpStream(port=5006)
tcpVidStream = tcpVideoStream()
...
def sendData(object_info):
    global tcpDataStream, noOfPeople
    base_index = 0
    ...
    if((label != "train") & (bottom > LowerBound) & (bottom < UpperBound)):
        tcpDataStream.send((1, detectedJson.encode()))
    else:
        tcpDataStream.send((3, detectedJson.encode()))
...
def main():
    ...
    try:
        while(True):
            ...
            tcpVidStream.send(display_image)
